...
def handle_order(message):
    ...
    # 🚫 Spam filter: Keywords and patterns to ignore
    spam_keywords = [
        "vpn", "🔥@speeeedvpnbot", "7 дней абсолютно бесплатно", "поддерживаются все устройства", "начать пробный период"
    ]
    if any(spam in text for spam in spam_keywords):
        logging.warning("⚠️ Spam message ignored: %s", message.text)
